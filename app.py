...
def board_commentary_paragraph_yearly(
    ...
    mcr_extreme_low = (mcr is not None and (mcr < 0.05))
    mcr_high = (mcr is not None and (mcr > 0.14))
    ...
    if margin_pressure:
        para += (
            " Gross margin yang relatif rendah menambah **area validasi kualitas margin** (pricing pressure atau biaya delivery tidak terkendali) sehingga kontrol scope, rework, dan disiplin komersial menjadi krusial."
        )
    ...
    if pd.notna(mcr):
        ...
        if mcr > 0.14:
            add(
                "critical",
                "MCR Critical (>14%)",
                f"MCR = {mcr:.1%}. Biaya manpower sudah berada pada level kritikal dan berpotensi menekan margin secara material.",
...
def board_decision_paragraph_yearly(
    ...
    narratives = {
        "HIGH_LEVERAGE": (
            ...
            "Prioritas governance adalah menjaga konsistensi delivery, memperkuat succession/knowledge transfer pada peran kunci, dan menetapkan guardrail kapasitas agar pertumbuhan tetap berkelanjutan. Jika gross margin masih tipis, lakukan margin quality review tanpa mengubah posture utama yang masih high leverage."
        ),
        "CAPACITY_RISK": (
...
def decision_badge_simple(rev_g, hc_g, rpe_g, mcr, gm=None, confidence: str = "Medium") -> Dict[str, str]:
    ...
    Gross Margin sengaja dipisahkan dari posture utama.
    Gross Margin dibaca melalui margin_quality_badge().
    """
    def ok(x):
    ...
    # HIGH LEVERAGE: growth kuat, RPE naik, HC terkendali, MCR masih sehat/watch rendah.
    if ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr <= 0.09:
        state = "HIGH_LEVERAGE"
    # CAPACITY RISK: growth dibeli dengan penambahan HC, produktivitas belum ikut naik.
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"
    # DEFENSIVE: revenue melemah/negatif.
    elif ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # COST PRESSURE: people-cost sudah mulai menekan.
    elif ok(mcr) and mcr > 0.09:
        state = "COST_PRESSURE"
...
def margin_quality_badge(gm, confidence: str = "Medium") -> Dict[str, str]:
    ...
    if not ok(gm):
        return {
            ...
            "tone": "info",
            "hint": "Gross Margin belum tersedia atau belum valid.",
        }
...
if business_badge.get("state") == "HIGH_LEVERAGE" and mq_state in ["WATCH", "PRESSURE", "CRITICAL"]:
    p2 += f" Margin Quality saat ini berada pada kategori **{mq_state}**, sehingga perlu evaluasi kualitas margin proyek/layanan tanpa mengubah posture utama yang masih **HIGH LEVERAGE**."
st.markdown(f"{p1}\n\n{p2}")
