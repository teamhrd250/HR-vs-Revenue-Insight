...
if load.mode == "YEARLY":
    ...
    badge = decision_badge_simple(latest.get("Revenue_YoY"), latest.get("Headcount_YoY"), latest.get("RPE_YoY"), latest.get("MCR_Pct"), latest.get("Gross_Margin_Pct"), confidence="Medium")
    render_decision_badge(badge)
    kpi_cards(kpis, cols=3)

    with st.expander("🧩 Contoh tampilan ideal (ringkas untuk Management)", expanded=False):
        st.markdown("""- **Ikon di judul KPI** membantu scanning cepat.
- **Decision badge** merangkum postur keputusan (High Leverage / Cost Pressure / Capacity Risk / Defensive).
- Gunakan badge sebagai *anchor*; detailkan lewat **Smart Insights** + **Board-Level Commentary**.""" )
