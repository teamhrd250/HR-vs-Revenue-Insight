...
with st.expander("📌 Board-Level Risk Commentary (Lengkap)", expanded=False):
    if board["disclaimers"]:
        st.warning(" / ".join(board["disclaimers"]))

    if board.get("alignment"):
        st.markdown("#### Sinkronisasi dengan Smart Insights")
        st.markdown("\n".join([f"- {x}" for x in board["alignment"]]))

    st.markdown("#### Key observations (fakta)")
    st.markdown("\n".join([f"- {x}" for x in board["observations"]]))

    st.markdown("#### Konsekuensi baik (upside / peluang)")
    st.markdown("\n".join([f"- {x}" for x in board.get("upsides", [])]) or "- —")

    st.markdown("#### Konsekuensi buruk (downside / risiko strategis)")
    st.markdown("\n".join([f"- {x}" for x in board["risks"]]))

    st.markdown("#### Guardrail & early warning (yang perlu dipantau)")
    st.markdown("\n".join([f"- {x}" for x in board.get("guardrails", [])]) or "- —")

    st.markdown("#### Board questions (yang perlu ditanyakan)")
    st.markdown("\n".join([f"- {x}" for x in board["questions"]]))

    st.markdown("#### Recommended actions (arah tindak lanjut)")
    st.markdown("\n".join([f"- {x}" for x in board["actions"]]))
# =========================
