...
def board_decision_paragraph_yearly(
    ...
    """Paragraf ke-2 (Decision Lens – Board Memo Style).
    - Tidak mengulang angka (angka sudah ada di KPI cards).
    - Fokus: pilihan keputusan Management, trade-off, dan mitigasi governance.
    """
    dq = dq or {}
    confidence = (dq.get("confidence") or "Medium").strip()

    # Minimal flags (tanpa menyebut angka di narasi)
    rev_g = kpi.get("revenue_growth")
    hc_g = kpi.get("headcount_growth")
    rpe_g = kpi.get("rpe_growth")
    mcr = kpi.get("mcr_latest")
    ...
    # Board-memo (tegas) — 2-4 kalimat
    p = []
    p.append(
        "Performa saat ini menunjukkan leverage organisasi yang kuat, namun dengan margin toleransi risiko yang semakin sempit."
        if high_leverage_growth
        else
        "Kondisi saat ini menuntut keseimbangan yang ketat antara pertumbuhan, kapasitas organisasi, dan kualitas eksekusi."
    )

    if extreme_efficiency:
        p.append(
            "Tanpa penguatan struktural yang tepat, risiko laten pada kapasitas—seperti ketergantungan individu, intensifikasi beban kerja, dan penurunan kualitas delivery—cenderung muncul tertunda dan berdampak tidak proporsional."
        )
    else:
        p.append(
            "Risiko utama berada pada ketahanan kapasitas dan konsistensi delivery; penguatan selektif lebih relevan daripada ekspansi organisasi yang agresif."
        )

    if margin_pressure:
        p.append(
            "Dengan indikasi tekanan margin, keputusan capacity reinforcement perlu diikat pada disiplin komersial dan kontrol kualitas eksekusi agar pertumbuhan tidak mengorbankan profitabilitas."
        )

    p.append(
        "Management perlu menentukan apakah strategi berikutnya berfokus pada optimalisasi struktur saat ini dengan guardrail ketat, atau penguatan selektif pada fungsi/peran kritikal untuk memastikan pertumbuhan yang berkelanjutan."
    )

    if confidence != "High":
        p.append("Catatan: confidence data belum maksimal; keputusan berbasis people-cost sebaiknya disertai verifikasi kelengkapan payroll/headcount per unit.")
