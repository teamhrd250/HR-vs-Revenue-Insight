...
def decision_badge_simple(rev_g, hc_g, rpe_g, mcr, gm=None, confidence: str = "Medium") -> Dict[str, str]:
    ...
    Catatan:
    - Gross Margin sengaja TIDAK dipakai sebagai penentu utama posture.
    - Gross Margin dibaca terpisah melalui Margin Quality badge.
    """
    def ok(x):
    ...
    # HIGH LEVERAGE: growth kuat, RPE naik, HC terkendali, dan MCR masih sehat/watch rendah.
    if ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr <= 0.09:
        state = "HIGH_LEVERAGE"
    # CAPACITY RISK: growth dibeli dengan tambahan HC, produktivitas belum menguat.
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"
    # DEFENSIVE: revenue melemah / negatif.
    elif ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # COST PRESSURE: people-cost melewati threshold, tetapi revenue belum negatif.
...
def margin_quality_badge(gm, confidence: str = "Medium") -> Dict[str, str]:
    ...
    if not ok(gm):
        return {
            ...
            "tone": "info",
            "hint": "Gross Margin belum tersedia / belum valid.",
        }
    ...
    if gm > 0.35:
        ...
        state, label, tone, hint = "HEALTHY", "🟢 HEALTHY", "success", "Margin sehat; tetap jaga kualitas delivery dan pricing."
    elif 0.15 <= gm < 0.25:
...
mq_state = margin_quality_badge(kpi_num.get("gross_margin_latest"), confidence=dq_now.get("confidence", "Medium")).get("state", "UNKNOWN")
if business_badge.get("state") == "HIGH_LEVERAGE" and mq_state in ["WATCH", "PRESSURE", "CRITICAL"]:
    p2 += f" Margin Quality saat ini berada pada kategori **{mq_state}**, sehingga perlu dilakukan evaluasi kualitas margin proyek/layanan tanpa mengubah posture utama yang masih **HIGH LEVERAGE**."
st.markdown(f"{p1}\n\n{p2}")
