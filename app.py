...
def board_commentary_paragraph_yearly(
    ...
    mcr_high = (mcr is not None and (mcr > 0.20))
...
def board_decision_paragraph_yearly(
    ...
    # 1) Defensive first: revenue tidak tumbuh / turun
    if ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # 2) Cost pressure: margin ketat / cost ratio membesar
    elif (ok(mcr) and mcr > 0.12) or (ok(gm) and gm < 0.25):
        state = "COST_PRESSURE"
    # 3) High leverage: revenue & RPE naik kuat, HC terkendali, cost ratio sehat
    elif ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr < 0.15:
        state = "HIGH_LEVERAGE"
    # 4) Capacity risk: tumbuh tapi “membeli growth” lewat headcount, produktivitas tidak naik
...
def anomaly_flags_yearly(yr: pd.DataFrame) -> List[Dict]:
    ...
    if pd.notna(latest.get("Headcount_YoY")) and float(latest["Headcount_YoY"]) > 0.25:
        flags.append({
            ...
            "why": f"Headcount YoY = {float(latest['Headcount_YoY']):.1%} (>20%).",
...
def decision_badge_simple(rev_g, hc_g, rpe_g, mcr, gm, confidence: str = "Medium") -> Dict[str, str]:
    """Decision badge ringkas untuk header Executive Summary (tidak mengulang Smart Insights)."""
    def ok(x):
    ...
    meta = {
        ...
        "CAPACITY_RISK": ("⚠️ CAPACITY RISK", "warning", "Growth cenderung “membeli kapasitas”; butuh kontrol produktivitas & kualitas."),
        "COST_PRESSURE": ("🛑 COST PRESSURE", "error", "Margin/people-cost menekan; prioritas disiplin biaya & unit economics."),
        "DEFENSIVE": ("🧯 DEFENSIVE", "error", "Revenue melemah; prioritas proteksi margin, cash, dan kapasitas kritikal."),
        "BALANCED": ("🟦 BALANCED", "info", "Ruang optimisasi selektif; hindari ekspansi menyeluruh tanpa bottleneck nyata."),
    }
