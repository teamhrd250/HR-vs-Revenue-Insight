...
def generate_recommendations_yearly(yr: pd.DataFrame, fg: pd.DataFrame) -> List[Dict]:
    ...
    if pd.notna(mcr):
        ...
        if mcr > 0.28:
            add(
                "critical",
                "MCR tinggi untuk model telecom/project IT (>28%)",
                f"MCR = {mcr:.1%}. Risiko margin tertekan; struktur organisasi mulai payroll-heavy.",
            ...
            add(
                "warning",
                "MCR berada di zona waspada (20–28%)",
                f"MCR = {mcr:.1%}. Masih dapat diterima untuk project-based IT, tetapi perlu kontrol ketat saat revenue melambat.",
            ...
            add(
                "good",
                "MCR sehat untuk telecom/project IT (8–20%)",
                f"MCR = {mcr:.1%}. Struktur biaya manpower relatif sehat.",
                "Pertahankan disiplin manpower cost; fokus ke retention, capability building, dan automasi proses agar leverage tetap naik.",
                owner="HR",
            )
        elif mcr < 0.08:
            add(
                "good",
                "MCR sangat rendah (<8%) — efisien, namun perlu monitor sustainability",
                f"MCR = {mcr:.1%}. Efisiensi sangat kuat, tetapi perlu dipastikan tidak berasal dari under-investment kapasitas atau beban kerja berlebih.",
                "Jaga guardrail kapasitas: monitor utilisasi/overtime, backlog, SLA, turnover role kritikal, dan kebutuhan hiring selektif.",
                owner="HR + COO",
            )
...
def board_decision_paragraph_yearly(
    ...
    elif (ok(mcr) and mcr > 0.20) or (ok(gm) and gm < 0.25):
        state = "COST_PRESSURE"
