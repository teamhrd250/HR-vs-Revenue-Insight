...
def board_decision_paragraph_yearly(
    ...
    # 1) High leverage first: growth kuat, RPE naik, HC terkendali, MCR <= 9%.
    #    Gross margin rendah tidak otomatis membuat status defensive jika growth & RPE sangat kuat.
    if ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr <= 0.09:
        state = "HIGH_LEVERAGE"
    # 2) Capacity risk: tumbuh tapi “membeli growth” lewat headcount, produktivitas tidak naik
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"
    # 3) Defensive: revenue tidak tumbuh / turun
    elif ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # 4) Cost pressure: MCR melewati 9% atau margin sangat rendah (<20%) setelah tidak masuk high leverage
    elif (ok(mcr) and mcr > 0.09) or (ok(gm) and gm < 0.20):
        state = "COST_PRESSURE"
...
def decision_badge_simple(rev_g, hc_g, rpe_g, mcr, gm, confidence: str = "Medium") -> Dict[str, str]:
    """Decision badge ringkas untuk header Executive Summary.

    Final logic:
    - HIGH_LEVERAGE diprioritaskan jika growth kuat, RPE naik, HC terkendali, dan MCR sehat.
    - Gross margin rendah sendiri tidak otomatis DEFENSIVE; ia menjadi margin watch/cost pressure jika growth tidak kuat.
    - DEFENSIVE hanya untuk kondisi revenue melemah/negatif.
    """
    def ok(x):
    ...
    # 1) High leverage first: revenue & RPE naik kuat, HC terkendali, MCR masih sehat/watch rendah.
    if ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr <= 0.09:
        state = "HIGH_LEVERAGE"
    # 2) Capacity risk: revenue tumbuh, tapi HC tumbuh lebih agresif dan produktivitas tidak naik.
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"
    # 3) Defensive: revenue tidak tumbuh/negatif.
    elif ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # 4) Cost pressure: MCR melewati watch/cost band atau margin rendah saat tidak masuk high leverage.
    elif (ok(mcr) and mcr > 0.09) or (ok(gm) and gm < 0.20):
        state = "COST_PRESSURE"

    meta = {
        "HIGH_LEVERAGE": ("✅ HIGH LEVERAGE", "success", "Growth scalable; fokus guardrail kapasitas & kualitas."),
        "CAPACITY_RISK": ("⚠️ CAPACITY RISK", "warning", "Growth mulai menekan kapasitas; butuh kontrol produktivitas & kualitas."),
        "COST_PRESSURE": ("🛑 COST PRESSURE", "warning", "Margin/people-cost perlu dikendalikan; cek unit economics dan delivery cost."),
        "DEFENSIVE": ("🧯 DEFENSIVE", "error", "Revenue melemah; prioritas proteksi margin, cash, dan kapasitas kritikal."),
        "BALANCED": ("🟦 BALANCED", "info", "Pertumbuhan dan kapasitas relatif seimbang; optimasi selektif."),
    }
...
_badge_color = _badge_color_map.get(badge.get("tone", "info"), "#7f8c8d")
    ...
    st.markdown(
        ...
        <div style="text-align:right; padding-top:6px;">
          <span style="
            ...
            background: rgba(255,255,255,0.04);
            font-size:12px;">
            <span style="width:10px; height:10px; border-radius:999px; background:{_badge_color}; display:inline-block;"></span>
            <span style="opacity:0.9;"><b>Decision:</b> {badge.get('state', 'BALANCED')}</span>
...
# Catatan kecil: badge membaca posture keputusan, bukan hanya 1 KPI.
if badge.get("state") == "HIGH_LEVERAGE" and gm_v is not None and gm_v < 0.25:
    st.caption("Decision State: HIGH LEVERAGE karena revenue & RPE tumbuh kuat dengan HC terkendali dan MCR sehat. Gross margin rendah tetap dibaca sebagai area validasi margin, bukan otomatis Defensive.")
