    ...
    if pd.notna(mcr):
        ...
        if mcr > 0.14:
            add(
                "critical",
                "MCR Critical (>14%)",
                f"MCR = {mcr:.1%}. Biaya manpower berada pada level kritikal dan berpotensi menekan margin secara material.",
                "Audit struktur manpower cost, overtime/bonus/benefit, serta evaluasi kontribusi per fungsi sebelum ekspansi lanjutan.",
                owner="HR + Finance + Management",
            )
        elif 0.09 < mcr <= 0.14:
            add(
                "warning",
                "MCR Cost Pressure (9–14%)",
                f"MCR = {mcr:.1%}. Biaya manpower mulai menekan profitabilitas dan perlu dikendalikan terhadap pertumbuhan revenue.",
                "Perketat hiring approval berbasis ROI, evaluasi staffing model, dan monitor overtime/bonus secara berkala.",
                owner="HR + Finance",
            )
        elif 0.07 < mcr <= 0.09:
            add(
                "warning",
                "MCR Watch (7–9%)",
                f"MCR = {mcr:.1%}. Biaya manpower mulai meningkat dan perlu dipantau agar tidak bergerak lebih cepat daripada revenue.",
                "Pantau tren MCR per periode, kaitkan hiring dengan kebutuhan delivery/revenue, dan jaga produktivitas per fungsi.",
                owner="HR + Finance",
            )
        elif 0.05 <= mcr <= 0.07:
            add(
                "good",
                "MCR Healthy (5–7%)",
                f"MCR = {mcr:.1%}. Struktur biaya manpower efisien dan masih sehat untuk mendukung profitabilitas.",
                "Pertahankan disiplin manpower cost; sustainability tetap divalidasi melalui indikator kapasitas seperti overtime, backlog, SLA, dan turnover role kritikal.",
                owner="HR + COO",
            )
        elif mcr < 0.05:
            add(
                "good",
                "MCR Ultra Efficiency (<5%) — validasi kapasitas diperlukan",
                f"MCR = {mcr:.1%}. Efisiensi manpower sangat tinggi; perlu dipastikan tidak berasal dari under-investment kapasitas atau beban kerja berlebih.",
                "Validasi sustainability melalui overtime, utilisasi, backlog, SLA/incident, turnover role kritikal, dan kebutuhan hiring selektif.",
                owner="HR + COO",
            )
...
def board_decision_paragraph_yearly(
    kpi: Dict[str, float],
    dq: Optional[Dict] = None,
) -> str:
    """Paragraf ke-2 (Decision Lens – Board Memo Style, state-driven).

    Tujuan:
    - Tidak mengulang angka (angka sudah ada di KPI cards & tren).
    - Menghasilkan narasi yang *berubah arah keputusan* ketika KPI berubah.
    - Fokus: pilihan keputusan Management, trade-off, dan guardrail governance.
    """
    dq = dq or {}
    confidence = (dq.get("confidence") or "Medium").strip()
    ...
    # =========================
    # BUSINESS POSTURE ENGINE
    # =========================
    state = "BALANCED"

    # Gross Margin tidak menentukan Business Posture; margin dibaca terpisah sebagai Margin Quality.
    if ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr <= 0.09:
        state = "HIGH_LEVERAGE"
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"
    elif ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    elif ok(mcr) and mcr > 0.09:
        state = "COST_PRESSURE"

    # =========================
    # STATE → NARRATIVE (TEGAS)
    # =========================
    narratives = {
        "HIGH_LEVERAGE": (
            "Management saat ini berada pada posisi leverage organisasi yang kuat, di mana pertumbuhan dicapai tanpa ekspansi kapasitas yang agresif. "
            "Arah keputusan sebaiknya menekankan stabilisasi kapasitas inti dan pengendalian risiko laten, bukan sekadar mempercepat rekrutmen. "
            "Prioritas governance adalah menjaga konsistensi delivery, memperkuat succession/knowledge transfer pada peran kunci, dan menetapkan guardrail kapasitas agar pertumbuhan tetap berkelanjutan."
        ),
        "CAPACITY_RISK": (
            "Pola pertumbuhan saat ini menunjukkan ketergantungan yang meningkat pada penambahan kapasitas, sementara penguatan produktivitas belum memadai. "
            "Management perlu memutuskan apakah akan melanjutkan ekspansi dengan pengetatan efektivitas (struktur peran, kompetensi, dan kontrol kualitas), atau menahan ekspansi untuk memastikan output per orang membaik terlebih dahulu. "
            "Tanpa disiplin ini, risiko yang muncul adalah pembesaran biaya tetap, friksi koordinasi, dan kualitas eksekusi yang tidak stabil."
        ),
        "COST_PRESSURE": (
            "Indikasi tekanan biaya/margin mempersempit ruang toleransi terhadap inefisiensi, sehingga keputusan utama Management perlu bergeser ke disiplin struktur biaya dan kualitas eksekusi. "
            "Sebelum mendorong pertumbuhan lanjutan, lakukan evaluasi kontribusi per fungsi, perketat guardrail people-cost, dan pastikan unit economics tidak terdegradasi. "
            "Keterlambatan intervensi berisiko mengunci organisasi pada struktur biaya yang kurang adaptif."
        ),
        "DEFENSIVE": (
            "Perlambatan kinerja menuntut pendekatan defensif dan selektif: lindungi margin, jaga cash discipline, dan prioritaskan fungsi yang menjaga pendapatan serta operasional inti. "
            "Management sebaiknya menahan ekspansi luas dan fokus pada stabilisasi kapasitas kritikal, perbaikan produktivitas, serta pengendalian risiko delivery. "
            "Tujuan fase ini adalah mengembalikan ketahanan sebelum kembali ke strategi pertumbuhan."
        ),
        "BALANCED": (
            "Kondisi saat ini relatif seimbang, memberikan ruang bagi Management untuk mengoptimalkan struktur yang ada sambil menjaga disiplin eksekusi. "
            "Arah keputusan yang sehat adalah penguatan selektif pada bottleneck nyata (bukan ekspansi menyeluruh), disertai guardrail kapasitas, kualitas delivery, dan pengendalian people-cost. "
            "Fokusnya memastikan pertumbuhan berikutnya bersifat stabil, bukan rapuh."
        ),
    }

    p2 = narratives.get(state, narratives["BALANCED"])

    if confidence != "High":
        p2 += " Catatan: confidence data belum maksimal; keputusan berbasis people-cost sebaiknya disertai verifikasi kelengkapan payroll/headcount per unit."
...
def data_quality_score_yearly(
    ...
    if "Dept_ID" in hc.columns and "Dept_ID" in dim.columns:
        ...
        unmapped_hc = {str(x).strip() for x in unmapped_hc if str(x).strip() not in ["", "nan", "None"]}
    ...
    if "Dept_ID" in pay.columns and "Dept_ID" in dim.columns:
        ...
        unmapped_pay = {str(x).strip() for x in unmapped_pay if str(x).strip() not in ["", "nan", "None"]}
    ...
    try:
        if file_bytes is None:
            if not default_path or not os.path.exists(default_path):
                return LoadResult(
                    ...
                    {},
                    [
                        f"File default tidak ditemukan: {default_path}",
                        "Upload file Excel (.xlsx), atau pastikan file default ada di folder data/ pada repo.",
                    ],
                )
            ...
            if not file_bytes:
                return LoadResult(
                    ...
                    {},
                    {},
                    ["File upload kosong. Upload ulang file Excel (.xlsx)."],
                )
        ...
        return LoadResult(
            ...
            [
                ...
                "Pastikan file adalah Excel Workbook (.xlsx) yang valid, bukan .csv/.xls lama/file corrupt.",
...
def decision_badge_simple(rev_g, hc_g, rpe_g, mcr, gm=None, confidence: str = "Medium") -> Dict[str, str]:
    """Business Posture badge untuk Executive Summary.

    Gross Margin sengaja tidak dijadikan penentu utama posture.
    Gross Margin dibaca terpisah melalui margin_quality_badge().
    """
    def ok(x):
    ...
    meta = {
        "HIGH_LEVERAGE": ("✅ HIGH LEVERAGE", "success", "Revenue & RPE tumbuh kuat dengan HC terkendali dan MCR sehat/watch rendah."),
        "BALANCED": ("🟦 BALANCED", "info", "Pertumbuhan, kapasitas, dan people-cost relatif seimbang; optimasi dilakukan selektif."),
        "CAPACITY_RISK": ("⚠️ CAPACITY RISK", "warning", "Pertumbuhan mulai menekan kapasitas; perlu validasi produktivitas dan bottleneck."),
        "COST_PRESSURE": ("🛑 COST PRESSURE", "warning", "People-cost mulai menekan; fokus pada disiplin biaya dan unit economics."),
        "DEFENSIVE": ("🧯 DEFENSIVE", "error", "Revenue melemah; prioritas proteksi margin, cash, dan fungsi kritikal."),
    }
    label, tone, hint = meta.get(state, meta["BALANCED"])
    if (confidence or "Medium") != "High":
        hint += " (confidence data belum High)"
    return {"state": state, "label": label, "tone": tone, "hint": hint}


def margin_quality_badge(gm, confidence: str = "Medium") -> Dict[str, str]:
    """Margin Quality badge terpisah dari Business Posture."""
    def ok(x):
        return x is not None and (x == x)

    if not ok(gm):
        return {"state": "UNKNOWN", "label": "⚪ MARGIN UNKNOWN", "tone": "info", "hint": "Gross Margin belum tersedia atau belum valid."}

    gm = float(gm)
    if gm > 0.35:
        state, label, tone, hint = "STRONG", "🟢 STRONG", "success", "Margin sangat kuat; ruang profitabilitas tinggi."
    elif 0.25 <= gm <= 0.35:
        state, label, tone, hint = "HEALTHY", "🟢 HEALTHY", "success", "Margin sehat; tetap jaga pricing, delivery cost, dan project mix."
    elif 0.15 <= gm < 0.25:
        state, label, tone, hint = "WATCH", "🟡 WATCH", "warning", "Margin perlu dipantau; validasi pricing, COGS, scope creep, dan biaya delivery."
    elif 0.10 <= gm < 0.15:
        state, label, tone, hint = "PRESSURE", "🟠 PRESSURE", "warning", "Margin mulai tertekan; perlu review profitabilitas proyek/layanan."
    else:
        state, label, tone, hint = "CRITICAL", "🔴 CRITICAL", "error", "Margin kritikal; perlu tindakan korektif pada pricing, cost delivery, atau project mix."
...
def capacity_risk_index(rev_g, hc_g, rpe_g, mcr) -> Dict[str, str]:
    """Capacity Risk Index untuk membaca sustainability dari leverage."""
    def ok(x):
        return x is not None and (x == x)
    ...
    if ok(rev_g) and ok(hc_g):
        gap = rev_g - hc_g
        if gap > 0.30:
            score += 2
            reasons.append("revenue tumbuh jauh di atas headcount")
        elif gap > 0.15:
            score += 1
            reasons.append("revenue tumbuh lebih cepat dari kapasitas")
        if hc_g > 0.15:
            score += 1
            reasons.append("headcount tumbuh agresif")
    if ok(rpe_g):
        if rpe_g > 0.30:
            score += 1
            ...
            score += 2
            reasons.append("RPE turun")
    if ok(mcr):
        if mcr < 0.05:
            score += 2
            reasons.append("MCR sangat rendah, perlu validasi kapasitas")
        elif mcr <= 0.07:
            score += 1
            reasons.append("MCR efisien dan perlu dijaga sustainability-nya")
        elif mcr > 0.14:
            score += 2
            reasons.append("MCR tinggi")
        elif mcr > 0.09:
            score += 1
            reasons.append("MCR masuk zona cost pressure")
    ...
    hint = "; ".join(reasons) if reasons else "Tidak ada sinyal kapasitas besar dari rule dasar."
    return {"state": state, "tone": tone, "score": score, "hint": hint}


def management_action_recommendation(business_state: str, margin_state: str, capacity_state: str) -> Dict[str, object]:
    """Management action otomatis untuk Board-level decision."""
    business_state = business_state or "BALANCED"
    margin_state = margin_state or "UNKNOWN"
    capacity_state = capacity_state or "LOW"

    if business_state == "HIGH_LEVERAGE":
        title = "Selective Scale-Up with Capacity Guardrail"
        actions = [
            "Pertahankan struktur HC yang disiplin; hindari ekspansi massal.",
            "Lakukan selective hiring hanya pada role bottleneck/revenue critical.",
            "Perkuat succession planning, knowledge transfer, dan retention role kunci.",
            "Pantau overtime, backlog, SLA/incident, dan turnover role kritikal."
        ]
        ...
        actions = [
            ...
            "Tetapkan threshold utilisasi dan SLA sebagai guardrail."
        ]
        ...
        actions = [
            ...
            "Tunda hiring non-critical sampai MCR kembali ke rentang sehat."
        ]
        ...
        actions = [
            ...
            "Realokasi kapasitas ke aktivitas yang langsung menjaga pendapatan."
        ]
    else:
        ...
        actions = [
            ...
            "Perkuat SOP, dokumentasi, dan governance delivery."
        ]

    if margin_state in ["WATCH", "PRESSURE", "CRITICAL"]:
        actions.append("Tambahkan margin quality review: pricing, COGS, scope creep, rework, dan delivery cost.")
    if capacity_state in ["HIGH", "CRITICAL"]:
        actions.append("Lakukan capacity risk review bulanan sampai risiko turun ke Medium/Low.")

    return {"title": title, "actions": actions}


def classify_business_posture_row(row: pd.Series) -> str:
    rev_g = row.get("Revenue_YoY")
    hc_g = row.get("Headcount_YoY")
    rpe_g = row.get("RPE_YoY")
    mcr = row.get("MCR_Pct")
    b = decision_badge_simple(
        float(rev_g) if pd.notna(rev_g) else None,
        float(hc_g) if pd.notna(hc_g) else None,
        float(rpe_g) if pd.notna(rpe_g) else None,
        float(mcr) if pd.notna(mcr) else None,
        None,
        confidence="High",
    )
    return b.get("state", "BALANCED")


def build_fja_mix(fg: pd.DataFrame) -> pd.DataFrame:
    """Functional Job Analysis mix berdasarkan Function_Group existing."""
    if fg is None or fg.empty:
        return pd.DataFrame(columns=["Year", "FJA_Category", "Manpower_Cost", "Headcount"])

    f = fg.copy()
    if "Function_Group" not in f.columns:
        f["Function_Group"] = "UNMAPPED"
    ...
    f["FJA_Category"] = f["Function_Group"].fillna("UNMAPPED").map(mapping).fillna("Support Function")

    out = f.groupby(["Year", "FJA_Category"], as_index=False).agg(
        Manpower_Cost=("Manpower_Cost", "sum"),
        Headcount=("Headcount", "sum"),
    )
    total_cost = out.groupby("Year")["Manpower_Cost"].transform("sum")
    total_hc = out.groupby("Year")["Headcount"].transform("sum")
    out["Cost_Share"] = np.where(total_cost > 0, out["Manpower_Cost"] / total_cost, np.nan)
    out["HC_Share"] = np.where(total_hc > 0, out["Headcount"] / total_hc, np.nan)
    return out


def render_decision_badge(b: Dict[str, str]):
    """Badge kecil yang rapi, ditempatkan dekat judul section."""
    tone = b.get("tone", "info")
    label = b.get("label", "🟦 BALANCED")
    hint = b.get("hint", "")
    if tone == "success":
...
with st.sidebar:
    ...
    use_default = st.checkbox("Gunakan file default dari repo", value=(uploaded is None), disabled=(uploaded is not None))
...
# Jika user upload Excel, selalu pakai file upload.
# Jika tidak upload, aplikasi mencoba membaca file default dari repo.
...
# Stop early if required sheets are missing, so later code does not crash with KeyError.
_required_now = YEARLY_SHEETS_REQUIRED if load.mode == "YEARLY" else MONTHLY_SHEETS_REQUIRED
_missing_now = [s for s in _required_now if s not in load.sheets]
if _missing_now:
    st.error("Sheet wajib belum lengkap: " + ", ".join(_missing_now))
    st.stop()
...
# =========================
# HITUNG OUTPUT (YEARLY / MONTHLY)
# =========================
if load.mode == "YEARLY":
    out = calc_yearly(load.sheets)
elif load.mode == "MONTHLY":
    out = calc_monthly(load.sheets)
else:
    st.error("Mode template tidak dikenali.")
    st.stop()

# Helper: KPI table
yr = out.get("yearly_kpi")
if yr is None:
    yr = out.get("yr")  # fallback (jika key berbeda)
if yr is None or yr.empty:
    st.error("Tabel KPI tahunan tidak tersedia / kosong.")
    st.stop()
yr = yr.copy()

# Sidebar filter (Year range) — muncul setelah data terbaca
years = sorted([int(x) for x in yr["Year"].dropna().unique()]) if "Year" in yr.columns else []
year_range = None
if years:
    y_min, y_max = min(years), max(years)
    # default: full range
    year_range = st.sidebar.slider("Year range", min_value=y_min, max_value=y_max, value=(y_min, y_max), step=1)
    yr = yr[(yr["Year"] >= year_range[0]) & (yr["Year"] <= year_range[1])].copy()

# Latest & previous year (untuk KPI card & delta)
yr_sorted = yr.sort_values("Year")
latest = yr_sorted.iloc[-1]
prev = yr_sorted.iloc[-2] if len(yr_sorted) >= 2 else None

# =========================
# EXECUTIVE SUMMARY (Yearly)
# =========================
st.markdown("## Executive Summary")

# Decision badge kecil (kanan atas)
rev_g = float(latest.get("Revenue_YoY")) if pd.notna(latest.get("Revenue_YoY")) else None
hc_g  = float(latest.get("Headcount_YoY")) if pd.notna(latest.get("Headcount_YoY")) else None
rpe_g = float(latest.get("RPE_YoY")) if pd.notna(latest.get("RPE_YoY")) else None
mcr_v = float(latest.get("MCR_Pct")) if pd.notna(latest.get("MCR_Pct")) else None
gm_v  = float(latest.get("Gross_Margin_Pct")) if pd.notna(latest.get("Gross_Margin_Pct")) else None

# Data quality (untuk confidence badge)
dq_now_local = data_quality_score_yearly(
    yr=yr,
...
business_badge = decision_badge_simple(
    rev_g, hc_g, rpe_g, mcr_v, gm_v,
    confidence=dq_now_local.get("confidence", "Medium"),
)
margin_badge = margin_quality_badge(
    gm_v,
    confidence=dq_now_local.get("confidence", "Medium"),
)
cap_badge = capacity_risk_index(rev_g, hc_g, rpe_g, mcr_v)
...
_business_color = _badge_color_map.get(business_badge.get("tone", "info"), "#7f8c8d")
_margin_color = _badge_color_map.get(margin_badge.get("tone", "info"), "#7f8c8d")
_capacity_color = _badge_color_map.get(cap_badge.get("tone", "info"), "#7f8c8d")
_badge_conf = dq_now_local.get("confidence", "Medium")
        ...
        <div style="text-align:right; padding-top:4px;">
          <div style="margin-bottom:6px;">
            <span style="
              ...
              background: rgba(255,255,255,0.04);
              font-size:12px;">
              <span style="width:10px; height:10px; border-radius:999px; background:{_business_color}; display:inline-block;"></span>
              <span style="opacity:0.9;"><b>Business Posture:</b> {business_badge.get('state', 'BALANCED')}</span>
              <span style="opacity:0.65;">({_badge_conf})</span>
            </span>
          </div>
          <div style="margin-bottom:6px;">
            <span style="
              ...
              background: rgba(255,255,255,0.04);
              font-size:12px;">
              <span style="width:10px; height:10px; border-radius:999px; background:{_margin_color}; display:inline-block;"></span>
              <span style="opacity:0.9;"><b>Margin Quality:</b> {margin_badge.get('state', 'UNKNOWN')}</span>
              <span style="opacity:0.65;">({_pct(gm_v)})</span>
            </span>
          </div>
          <div>
            <span style="
              ...
              background: rgba(255,255,255,0.04);
              font-size:12px;">
              <span style="width:10px; height:10px; border-radius:999px; background:{_capacity_color}; display:inline-block;"></span>
              <span style="opacity:0.9;"><b>Capacity Risk:</b> {cap_badge.get('state', 'LOW')}</span>
            </span>
          </div>
        </div>
        """,
...
st.caption(
    f"Business Posture: {business_badge.get('hint','-')} "
    f"Margin Quality: {margin_badge.get('hint','-')} "
    f"Capacity Risk: {cap_badge.get('hint','-')}"
)

# KPI cards dengan ikon (lebih cepat discan)
...
def _pp(x):
    if x is None or pd.isna(x):
        return ""
    return f"{x*100:.1f} pp" if abs(x) < 1 else f"{x:.1f} pp"

with c1:
    c1.metric("💰 Total Revenue (latest)", _money(latest.get("Total_Revenue")), _pct(latest.get("Revenue_YoY")))
with c2:
    c2.metric("👥 Total Headcount (avg)", f"{latest.get('Total_Headcount'):.1f}" if pd.notna(latest.get("Total_Headcount")) else "-", _pct(latest.get("Headcount_YoY")))
with c3:
    c3.metric("⚡ RPE (Revenue/Employee)", _money(latest.get("RPE")), _pct(latest.get("RPE_YoY")))

with c4:
    # delta MCR gunakan MCR_Delta jika ada (pp), fallback kosong
    mcr_delta = latest.get("MCR_Delta")
    d = f"{mcr_delta*100:+.1f} pp" if pd.notna(mcr_delta) else ""
    c4.metric("📊 Manpower Cost Ratio (MCR)", _pct(latest.get("MCR_Pct")), d)
with c5:
    # Gross margin YoY jika ada
    gm_yoy = latest.get("GrossMargin_YoY")
    gm_delta = _pct(gm_yoy) if pd.notna(gm_yoy) else ""
    c5.metric("📈 Gross Margin", _pct(latest.get("Gross_Margin_Pct")), gm_delta)
with c6:
    c6.metric("💼 Total Manpower Cost", _money(latest.get("Total_Manpower_Cost")), _pct(latest.get("ManpowerCost_YoY")))
...
st.markdown("#### 📌 Management Risk Commentary")
p1 = board_commentary_paragraph_yearly(kpi=kpi_num, dq=dq_now, smart_recs=dengan_smart.get("recs"))
p2 = board_decision_paragraph_yearly(kpi=kpi_num, dq=dq_now)
mq_state = margin_quality_badge(
    kpi_num.get("gross_margin_latest"),
    confidence=dq_now.get("confidence", "Medium")
).get("state", "UNKNOWN")
if business_badge.get("state") == "HIGH_LEVERAGE" and mq_state in ["WATCH", "PRESSURE", "CRITICAL"]:
    p2 += f" Margin Quality saat ini berada pada kategori **{mq_state}**, sehingga perlu evaluasi kualitas margin proyek/layanan tanpa mengubah posture utama yang masih **HIGH_LEVERAGE**."
st.markdown(f"{p1}\n\n{p2}")

# =========================
# BOARD DECISION ADD-ONS
# =========================
st.markdown("### 🧭 Board Decision Add-ons")

tab_fja, tab_capacity, tab_trend, tab_action = st.tabs([
    "🏗 Functional Cost Mix (FJA)",
    "⚡ Capacity Risk Index",
...
with tab_fja:
    fja_mix = build_fja_mix(out.get("fg_breakdown", pd.DataFrame()))
    if year_range is not None and not fja_mix.empty:
        fja_mix = fja_mix[(fja_mix["Year"] >= year_range[0]) & (fja_mix["Year"] <= year_range[1])].copy()

    latest_fja_year = int(fja_mix["Year"].dropna().max()) if not fja_mix.empty and fja_mix["Year"].notna().any() else None
    if latest_fja_year is not None:
        fja_latest = fja_mix[fja_mix["Year"] == latest_fja_year].copy()
        c_fja1, c_fja2 = st.columns([1.2, 1.0])
        with c_fja1:
            fig = px.treemap(
                fja_latest,
                path=["FJA_Category"],
                values="Manpower_Cost",
                title=f"Functional Cost Mix by FJA Category — {latest_fja_year}",
            )
            fig.update_layout(margin=dict(l=10, r=10, t=50, b=10))
            st.plotly_chart(fig, use_container_width=True)
        with c_fja2:
            show_fja = fja_latest[["FJA_Category", "Manpower_Cost", "Cost_Share", "Headcount", "HC_Share"]].copy()
            show_fja["Cost_Share"] = show_fja["Cost_Share"].map(lambda x: f"{x:.1%}" if pd.notna(x) else "-")
            show_fja["HC_Share"] = show_fja["HC_Share"].map(lambda x: f"{x:.1%}" if pd.notna(x) else "-")
            st.dataframe(show_fja.sort_values("Manpower_Cost", ascending=False), use_container_width=True, hide_index=True)
        st.caption("FJA memakai mapping otomatis dari Function_Group: Sales = Revenue Generator; Operations/Project & Engineering = Revenue Enabler; Support = Support Function; Management = Governance/Management.")
    else:
        st.info("FJA mix belum tersedia karena data function group/cost kosong.")

with tab_capacity:
    st.metric("Capacity Risk Index", cap_badge.get("state", "LOW"), f"Score {cap_badge.get('score', 0)}")
    st.write(cap_badge.get("hint", "-"))
    st.markdown(
        "**Cara baca:** Capacity Risk bukan berarti pasti burnout. Ini early warning untuk memvalidasi sustainability melalui overtime, utilisasi, backlog, SLA/incident, turnover role kritikal, dan dependency pada key person."
    )

with tab_trend:
    trend = yr.sort_values("Year").copy()
    trend["Business_Posture"] = trend.apply(classify_business_posture_row, axis=1)
    trend["Margin_Quality"] = trend["Gross_Margin_Pct"].apply(lambda x: margin_quality_badge(float(x) if pd.notna(x) else None).get("state", "UNKNOWN"))
    trend_show = trend[["Year", "Business_Posture", "Margin_Quality", "Revenue_YoY", "Headcount_YoY", "RPE_YoY", "MCR_Pct", "Gross_Margin_Pct"]].copy()

    trend_show["Bubble_Size"] = trend_show["Revenue_YoY"].fillna(0).abs() + 0.05
    fig = px.scatter(
        trend_show,
        x="Year",
        y="Business_Posture",
        ...
        title="Decision State Trend — Business Posture vs Margin Quality",
    )
    fig.update_layout(margin=dict(l=10, r=10, t=50, b=10))
    st.plotly_chart(fig, use_container_width=True)
    st.dataframe(trend_show.drop(columns=["Bubble_Size"]), use_container_width=True, hide_index=True)

with tab_action:
    action = management_action_recommendation(
        business_badge.get("state", "BALANCED"),
        margin_badge.get("state", "UNKNOWN"),
        cap_badge.get("state", "LOW"),
    )
    st.markdown(f"#### {action['title']}")
    for i, a in enumerate(action["actions"], start=1):
        st.write(f"{i}. {a}")
...
with c1r:
    ...
    st.dataframe(dept_view.sort_values("Manpower_Cost", ascending=False).head(top_n) if "Manpower_Cost" in dept_view.columns else dept_view.head(top_n), use_container_width=True, hide_index=True)

with c2r:
    st.write("Top Headcount")
    st.dataframe(dept_view.sort_values("Headcount", ascending=False).head(top_n) if "Headcount" in dept_view.columns else dept_view.head(top_n), use_container_width=True, hide_index=True)

with c3r:
    st.write("Highest Cost per HC")
    st.dataframe(dept_view.sort_values("Cost_per_HC", ascending=False).head(top_n) if "Cost_per_HC" in dept_view.columns else dept_view.head(top_n), use_container_width=True, hide_index=True)
...
if "Headcount" in dept_tbl.columns and "Manpower_Cost" in dept_tbl.columns:
    ...
    show_debug = st.checkbox("Tampilkan tabel KPI tahunan (debug / export)", value=False)
    if show_debug:
        show = yr.copy()
        st.dataframe(show, use_container_width=True, hide_index=True)


if load.mode == "MONTHLY":
    ...
    show_debug_m = st.checkbox("Tampilkan tabel KPI monthly/yearly (debug)", value=False)
    if show_debug_m:
