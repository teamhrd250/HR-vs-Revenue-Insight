...
def decision_state_engine_yearly(kpi: Dict[str, float], dq: Optional[Dict] = None) -> Dict[str, str]:
    """Return decision state label + color for a small badge in Executive Summary."""
    dq = dq or {}
    confidence = (dq.get("confidence") or "Medium").strip()
    ...
    # Defensive first: revenue tidak tumbuh / turun
    if ok(rev_g) and rev_g <= 0:
        state = "DEFENSIVE"
    # Cost pressure: margin ketat / cost ratio membesar
    elif (ok(mcr) and mcr > 0.20) or (ok(gm) and gm < 0.25):
        state = "COST_PRESSURE"
    # High leverage: revenue & RPE naik kuat, HC terkendali, cost ratio sehat
    elif ok(rev_g) and ok(rpe_g) and ok(hc_g) and ok(mcr) and rev_g > 0.20 and rpe_g > 0.15 and hc_g < 0.10 and mcr < 0.15:
        state = "HIGH_LEVERAGE"
    # Capacity risk: tumbuh tapi “membeli growth” lewat headcount, produktivitas tidak naik
    elif ok(rev_g) and ok(hc_g) and rev_g > 0.10 and hc_g > 0.15 and (not ok(rpe_g) or rpe_g <= 0.0):
        state = "CAPACITY_RISK"

    meta = {
        "HIGH_LEVERAGE": ("🟢 Decision State: HIGH LEVERAGE", "#16a34a"),
        "CAPACITY_RISK": ("🟠 Decision State: CAPACITY RISK", "#f59e0b"),
        "COST_PRESSURE": ("🔴 Decision State: COST PRESSURE", "#ef4444"),
        "DEFENSIVE": ("🔴 Decision State: DEFENSIVE", "#ef4444"),
        "BALANCED": ("🟡 Decision State: BALANCED", "#eab308"),
    }
    label, color = meta.get(state, meta["BALANCED"])

    note = ""
    if confidence != "High":
        note = f"Confidence: {confidence}"

    return {"state": state, "label": label, "color": color, "note": note}


def render_decision_badge(badge: Dict[str, str]) -> None:
    """Render a small pill badge aligned to the right."""
    label = badge.get("label", "Decision State")
    ...
    st.markdown(
        ...
        <div style="display:flex; justify-content:flex-end; margin-top:-6px; margin-bottom:8px;">
          ...
          ">
            ...
            {f'<span style="opacity:0.7; font-weight:500;">· {note}</span>' if note else ''}
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
if load.mode == "YEARLY":
    ...
    # Badge kecil (Decision State) — ringkas untuk arah keputusan Management
    badge = decision_state_engine_yearly(kpi=kpi_num, dq=dq_now)
    ...
    with st.expander("🧩 Contoh tampilan ideal (ringkas untuk Management)", expanded=False):
        st.markdown(
            """- **Ikon di judul KPI** membantu scanning cepat (Revenue/HC/RPE/MCR/GM/People Cost).
- **Decision badge** merangkum “postur keputusan” (mis. *High Leverage / Cost Pressure / Capacity Risk / Defensive*).
- Rekomendasi: gunakan badge ini sebagai *anchor*, lalu detailnya lihat di **Smart Insights** + **Board-Level Commentary**."""
        )
