...
def board_decision_paragraph_yearly(
    ...
    """Paragraf ke-2 (Board Decision Lens) — tidak mengulang angka.
    Fokus: opsi keputusan pimpinan, trade-off, dan governance guardrails.
    """
    dq = dq or {}
    ...
    # Flags (tanpa angka di narasi)
    high_leverage_growth = (
        rev_g is not None and hc_g is not None and rpe_g is not None
        and (rev_g == rev_g) and (hc_g == hc_g) and (rpe_g == rpe_g)  # not NaN
        and rev_g > 0.30 and hc_g < 0.05 and rpe_g > 0.25
    )
    extreme_efficiency = (mcr is not None and (mcr == mcr) and mcr < 0.06)
    margin_pressure = (gm is not None and (gm == gm) and gm < 0.30)

    # Build decision-focused paragraph (no angka)
    parts = []
    parts.append(
        "Implikasi untuk keputusan pimpinan: organisasi berada pada fase leverage yang kuat, "
        "sehingga fokus keputusan bergeser dari sekadar efisiensi biaya menuju ketahanan kapasitas dan kualitas eksekusi."
        if high_leverage_growth
        else
        "Implikasi untuk keputusan pimpinan: fokus utama adalah menyeimbangkan pertumbuhan, kapasitas organisasi, dan kualitas eksekusi."
    )

    # Trade-off framing
    if extreme_efficiency:
        parts.append(
            "Pilihan strategis yang perlu diputuskan adalah apakah perusahaan akan mempertahankan struktur yang sangat ramping "
            "untuk memaksimalkan leverage jangka pendek, atau melakukan penguatan selektif pada peran/fungsi kritikal "
            "untuk menurunkan risiko burnout, single-point-of-failure, dan penurunan kualitas delivery yang biasanya muncul tertunda."
        )
    else:
        parts.append(
            "Pimpinan dapat menetapkan prioritas penguatan pada area yang paling mempengaruhi delivery dan pertumbuhan, "
            "tanpa memperbesar organisasi secara agresif."
        )

    # Margin lens
    if margin_pressure:
        parts.append(
            "Dengan adanya indikasi tekanan margin, keputusan penguatan kapasitas perlu dikaitkan dengan disiplin komersial "
            "(bid governance, pricing discipline) dan kontrol kualitas eksekusi, agar pertumbuhan tidak mengorbankan profitabilitas."
        )

    # Governance guardrails
    parts.append(
        "Mitigasi yang bersifat governance: tetapkan guardrail kapasitas & kualitas (mis. utilisasi/overtime, absenteeism, turnover per peran kritikal, "
        "backlog serta SLA/incident), dan pastikan succession coverage serta knowledge transfer pada posisi kunci."
    )

    if confidence != "High":
        parts.append(
            "Catatan: karena confidence data belum maksimal, keputusan berbasis people-cost sebaiknya disertai verifikasi kelengkapan payroll/headcount per unit."
        )
